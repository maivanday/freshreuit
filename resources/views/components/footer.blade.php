<!-- begin footer -->
@section('css')




@endsection
<!--  -->
<div class="container-fluid mt-5 ">
    <div class="row">

        <div class="carousel-item active">
            <img src="{{asset('frontend/dist/img/banner_4.jpg')}}" alt="" width="100%" height="450px" />
            <div id="top" class="carousel-caption">
                <h2 class="display-4">Fresh Fruit</h2>

                <h2 class="display-2">Ưu đãi đặt biệt</h2>
                <button type="button" class="btn btn-primary btn-lg">
                    -50%
                </button>
            </div>
        </div>
    </div>
</div>
<!--  -->


<div class="container-fluid pt-5">
    <div class="row ">
        <div class="col-md-3">
            <h3 class="footer_title"> GIỚI THIỆU </h3>

            <li class="footer-link">
                <a href="">Giới thiệu về FreshFruit </a>
            </li>
            <li class="footer-link">
                <a href="">Chính sách bảo mật thanh toán</a>
            </li>
            <li class="footer-link">
                <a href="">Chính sách bảo mật thông tin cá nhân</a>
            </li>
            <li class="footer-link">
                <a href="">Chính sách giải quyết khiếu nai</a>
            </li>
            <li class="footer-link">
                <a href="">Điều khoản và sử dụng</a>
            </li>

        </div>
        <div class="col-md-3">
            <h3 class="footer_title">CHĂM SÓC KHÁCH HÀNG </h3>
            <li class="footer-link">
                <a href="">Hướng dẫn đặt hàng</a>
            </li>
            <li class="footer-link">
                <a href="">Chính sách đổi trả</a>
            </li>
            <li class="footer-link">
                <a href="">Hướng dẫn mua trả góp</a>
            </li>
            <li class="footer-link">
                <a href="">Chính sách hàng nhập khẩu</a>
            </li>
            <li class="footer-link">
                <p>Hỗ trợ khách hàng: <a href="">hotro@freshfruit.vn</a></p>
            </li>
            <li class="footer-link">
                <p>Báo lỗi bảo mật: <a href="">security@freshfruit.vn</a></p>
            </li>

        </div>
        <div class="col-md-3">
            <h3 class="footer_title"> ĐỐI TÁC VẬN CHUYỂN </h3>
            <li class="footer-link">
                <a href=""> <img class="set_width" src="{{ asset('frontend/dist/img/footer1.png')}}" alt=""></a>
            </li>
            <li class="footer-link">
                <a href=""><img class="set_width" src="{{ asset('frontend/dist/img/footer2.png')}}"></a>
            </li>
            <li class="footer-link">
                <a href=""><img class="set_width" src="{{ asset('frontend/dist/img/footer3.png')}}" alt=""></a>
            </li>
            <li class="footer-link">
                <a href=""><img class="set_width" src="{{ asset('frontend/dist/img/footer4.png')}}" alt=""></a>
            </li>
            <li class="footer-link">
                <a href=""><img class="set_width" src="{{ asset('frontend/dist/img/footer5.png')}}" alt=""></a>

            </li>


        </div>
        <!-- <div class="col-2-4">
                        <h3 class="footer_title">PHƯƠNG THỨC THANH TOÁN </h3>
                        <li class="footer-link">
                            <a href=""> <img class="set_width" src="//media3.scdn.vn/img2/2018/7_27/ExHghW.png" alt=""></a>
                        </li>
                        <li class="footer-link">
                            <a href=""><img class="set_width" src="//media3.scdn.vn/img2/2018/7_27/6HLfiO.png" alt=""></a>
                        </li>
                        <li class="footer-link">
                            <a href=""><img class="set_width" src="//media3.scdn.vn/img2/2018/7_27/ZyrCMU.png" alt=""></a>
                        </li>
                        <li class="footer-link">
                            <a href=""><img class="set_width" src="///media3.scdn.vn/img2/2018/7_27/BN05j2.png" alt=""></a>

                        </li>
                    </div> -->
        <div class="col-md-3">
            <h3 class="footer_title">KẾT NỐI VỚI CHÚNG TÔI</h3>
            <li class="footer-link">
                <a href="">

                    facebook
                </a>
            </li>
            <li class="footer-link">
                <a href="">


                    instagram
                </a>
            </li>
            <li class="footer-link">
                <a href="">

                    twitter
                </a>


            </li>


        </div>
    </div>

</div>

<div class="container-fluid ">
    <div class="row ">
        <div class="col-12 copyright pl-0 pr-0">

            <p class="bg-success text-white">@2020 Bản quyền thuộc về FreshFruit</p>

        </div>
    </div>
</div>


<!-- end footer -->
